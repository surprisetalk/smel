...
func TestScrapscript(t *testing.T) {
	tests := []struct {
		...
		{in: `(| #x _ -> 234 | #a 123 -> 456 | _ -> 789) (#a 123)`, eval: "456"},
		{in: `(| #x _ -> 234 | #a   _ -> 456 | _ -> 789) (#b 123)`, eval: "789"},
		{in: `(| #x _ -> 234 | #a   _ -> 456 | _ -> 789) (#a 123)`, eval: "456"},
		...
		{in: `f [2, 4, 6] . f = | [] -> 0 | x >+ _ -> x | c -> 1`, print: `f [ 2, 4, 6 ] . f = | [] -> 0 | x >+ _ -> x | c -> 1`},
		{in: "1 < 2 && 2 < 1", print: "1 < 2 && 2 < 1"},
		{in: "false || false && true", print: "false || false && true", eval: "false"},
		{in: "#false () || #false () && #true ()", print: "#false () || #false () && #true ()", eval: "false"},
		{in: "#true () || #true () && #false ()", print: "#true () || #true () && #false ()", eval: "true"},
		...
		{in: "true ? false", error: "assertion failed"},
		...
		{in: `rest -> { name = "test", fn = x -> x + 1, data = [1, 2] ++ rest, sub = {x = 1} }`},
		{in: "f = | 0 -> 1\n | n -> n * f(n-1)"},
		{in: "1 \t\n\r \n\t +\t\n \t2"},
		{in: "1\n\n\n+\n\n\n2"},
		{in: "{x=[1,{y=2}]}"},
		...
		{in: "{x={y=1}}"},
		{in: "-0", eval: "0"},
		{in: "[ 1 , -1 , 1.0, -1.0 ]", eval: "[ 1, -1, 1.0, -1.0 ]"},
		{in: "x -> rest -> { x, ..rest }"},
		{in: "| 1 -> 2"},
		{in: "x -> x + 1"},
		{in: "{x=1,y=2}"},
		{in: "{x=1}"},
		{in: "{}"},
		{in: "-- comment1\n2\n-- comment3", parse: "2", print: "2", eval: "2"},
		{in: "-- 1\n2"},
		{in: "1  \n\n\t  +  \t\n  2"},
		{in: "1\n+\t2"},
		{in: "[[1,2],[3,4]]"},
		...
		{in: "g . g = | 1 -> 2 | 2 -> 3"},
		// {in: "a!b"},
		{in: "r@a . r = { a = 1 }", parse: "{32 [{61 r} map[a:1] {43 =} {61 r} {61 a} {43 @} {43 .}]}", print: "r@a . r = { a = 1 }", eval: "1"},
		{in: "123 -> {x=1,..{}}", print: "123 -> { ..{}, x = 1 }"},
		{in: "123 -> { x = 1, ..{} }", print: "123 -> { ..{}, x = 1 }"},
		{in: "a -> {x=1,..a}", print: "a -> { ..a, x = 1 }"},
		{in: "a -> { x = 1, ..a }", print: "a -> { ..a, x = 1 }"},
		{in: "{x=1,...} -> 123", print: "{ ..., x = 1 } -> 123"},
		{in: "{ x = 1, ... } -> 123", print: "{ ..., x = 1 } -> 123"},
		// {in: "~="},
		// {in: "~"},
		// {in: "a : b"},
